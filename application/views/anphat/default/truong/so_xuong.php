...
<?php endif;?>>
  ...
  <input class="form-control" type="text" placeholder="Tìm kiếm" ng-model="tim_kiem_<?= $ban_ghi_cap_nhat ?>_<?= $model? $model : $index ?>" />
...
<?php endif;?>>
    ...
    <option ng-value="<?= $option_value?>" ng-repeat="<?= $repeat?> | filter: tim_kiem_<?= $ban_ghi_cap_nhat ?>_<?= $model? $model : $index ?>">{{<?= $option_label?>}}</option>
  </select>
</div>