    ...
    <div class="row" style="width: 600px;">
      ...
      <label>Chọn Lớp</label>
        <select class="form-control form-control-sm" placeholder="Lớp" ng-model="<?= $class_selector_model?>" ng-change="<?= $class_selector_change?>">
          ...
          <option ng-repeat="cl in classes | filter:{'level':<?= $class_selector_model_alias?>_selected_level, 'teacherId': <?= $class_selector_model_alias?>_selected_teacher_id, 'status': <?= $class_selector_model_alias?>_selected_status, 'online': <?= $class_selector_model_alias?>_selected_online}" ng-value="cl.id">{{cl.name}}</option>
        </select>
      </div>
    </div>
    </div>
