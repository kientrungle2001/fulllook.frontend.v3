...
<div class="b_top">
	<h2><a itemprop="url" href="<?= $links_model->get_product_category_link($language, $third_category);?>"><?= wpglobus($third_category['name'], $language) ?></a></h2>
</div>
<br>
<!--content-box-->
<?php foreach ($posts as $post) :
	...
	<div class="cate2_s item_cate2 h-product" itemtype="http://schema.org/Product">
		<a itemprop="url" href="<?= $links_model->get_product_link($language, $third_category, $post)?>">
			<?php if ($img) : ?>
		...
		<h3><a href="<?= $links_model->get_product_link($language, $third_category, $post)?>" class="name_cate2 p-name" itemprop="name"><?= wpglobus($post['post_title'], $language) ?></a></h3>
		<br clear="all">
	</div>
<?php endforeach; ?>
