    ...
    <div class="p-3">
      <div class="row" style="width: 600px;">
        ...
        <div class="form-group col-24">
          ...
          <?php if (@$class_selector_type !== 'table') : ?>
            <select class="form-control form-control-sm" placeholder="Lớp" ng-model="<?= $class_selector_model ?>" ng-change="<?= $class_selector_change ?>">
          ...
          <table class="table">
            ...
            <tr ng-class="{'table-success': cl.id==classId}" ng-repeat="cl in classes | filter:{'level':<?= $class_selector_model_alias ?>_selected_level, 
          'subjectId': <?= $class_selector_model_alias ?>_selected_subject_id,
          'teacherId': <?= $class_selector_model_alias ?>_selected_teacher_id, 
          'status': <?= $class_selector_model_alias ?>_selected_status, 
          'online': <?= $class_selector_model_alias ?>_selected_online}">
              <td><a href="#" onclick="return false" ng-click="select_item('<?= $class_selector_model ?>', cl.id); <?= $class_selector_change ? $class_selector_change: '' ?>">{{cl.name}}</a></td>
              <td>{{cl.level}}</td>
