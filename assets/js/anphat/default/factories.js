...
anphatApp.factory("xoa_nhieu_ban_ghi", function() {
	return function(params, $callback) {
		auth_del({
			url: APC.api.v1.tong_quat.url,
			data: {
			...
			function(tong_quat) {
				auth_get_one(
					...
					function(loai_thuoc_tinh) {
						var _ids = [];
						if (tong_quat) _ids.push(tong_quat._id.$oid);
						if (loai_thuoc_tinh) _ids.push(loai_thuoc_tinh._id.$oid);
						if (_ids.length) {
							auth_get_many(
								"loai_thuoc_tinh_tham_so",
								{
									trang_thai: true,
									id_loai_thuoc_tinh: {
										$in: _ids
