...
<channel>
	...
	<item>
		...
		<link><?= SITE_PROTOCOL?><?= $_SERVER['HTTP_HOST']?><?= $controller->links_model->get_product_link($language, $category, $post)?></link>
		<guid isPermaLink="false"><?= SITE_PROTOCOL?><?= $_SERVER['HTTP_HOST']?>/product/detail/<?= $language?>/<?= $category['term_id']?>/<?= $post['ID']?></guid>
